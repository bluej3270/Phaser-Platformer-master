...
function create() {
  gameState.player = this.physics.add.sprite(650, 450, 'codey').setScale(.5);

  const platforms = this.physics.add.staticGroup();

	platforms.create(550, 400, 'platform').setScale(2, .1).refreshBody();

	platforms.create(650, 500, 'platform').setScale(.25, .3).refreshBody();
	platforms.create(450, 300, 'platform').setScale(.35, .1).refreshBody();
	platforms.create(450, 200, 'platform').setScale(.35, .1).refreshBody();
	platforms.create(200, 150, 'platform').setScale(.15, .1).refreshBody();
	platforms.create(700, 150, 'platform').setScale(.15, .1).refreshBody();
	...
	gameState.starOne = this.physics.add.sprite(200, 50, 'star').setScale(.15);
	this.physics.add.collider(gameState.starOne, gameState.player);
	this.physics.add.collider(gameState.starOne, platforms);

	gameState.starTwo = this.physics.add.sprite(700, 50, 'star').setScale(.15);
